...
import { deleteEmployeeData, employeeListData } from "redux/api/services/employeeService";
...
const Employees = () => {
    ...
    const stateValues = useSelector((state) => {
        return {
            ...
            employeeListData: state.employee.listEmployee
        }
    })
    ...
    const employeeListApi = async () => {
        const parameter = {
            url: `${authEndPoints.employee.employeeList}?per_page=10&page=${page}&search=${searchKey}`,

        };
        try {
            await dispatch(employeeListData(parameter)).unwrap();
        } catch (errors) {
            errorAlert(errors?.error);
        }
    };
    ...
    const handleButtonClick = async () => {
        ...
        await employeeListApi(); // Call handleAddDirectory to add directory data
    };
