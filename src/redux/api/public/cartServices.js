...
export const addCartServices = createAsyncThunk("addCartServices", async (params, thunkApi) => {
  ...
  const url = type === 'add' ? '/customer/cart/save' : `/customer/cart/decrement/${product_id}`
...
export const guestAddCartServices = createAsyncThunk('guestAddCartServices', async (params, thunkApi) => {
  const { type, product_id, cart_id } = params
  const data = {
    product_id,
    cart_id,
  ...
  const url = type === 'add' ? '/cart/save' : `/customer/cart/decrement/${product_id}`
  const method = type === "add" ? "POST" : "PUT"
  try {
    const response = await PUBLICSERVER({
      url,
...
export const removeCartServices = createAsyncThunk("removeCartServices", async (id = '', thunkApi) => {
